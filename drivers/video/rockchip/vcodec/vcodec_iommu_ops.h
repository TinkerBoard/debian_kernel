...
struct vcodec_iommu_ops {
	...
	int (*import)(struct vcodec_iommu_info *iommu_info, int fd);
	int (*free)(struct vcodec_iommu_info *iommu_info, int idx);
	uint8_t *(*map_kernel)(struct vcodec_iommu_info *iommu_info, int idx);
	int (*unmap_kernel)(struct vcodec_iommu_info *iommu_info, int idx);
	int (*map_iommu)(struct vcodec_iommu_info *iommu_info, int idx,
			 unsigned long *iova,
			 unsigned long *size);
	int (*unmap_iommu)(struct vcodec_iommu_info *iommu_info, int idx);
	int (*destroy)(struct vcodec_iommu_info *iommu_info);
	void (*dump)(struct vcodec_iommu_info *iommu_info);
