...
static u32 cif_isp10_pix_fmt2v4l2_pix_fmt(
	enum cif_isp10_pix_fmt cif_pix_fmt)
{
	switch (cif_pix_fmt) {
	case CIF_YUV400:
	...
	default:
		cif_isp10_pltfrm_pr_err(NULL,
			"unknown or unsupported CIF pixel format %d\n",
				cif_pix_fmt);
		return 0;
	}
}

int cif_isp10_v4l2_fmt_strm_fmt(
	...
	struct v4l2_format *f)
{
	...
	ifmt = get_cif_isp10_output_format_index(f->fmt.pix.pixelformat);
	...
	if (CIF_ISP10_PIX_FMT_IS_YUV(strm_fmt->frm_fmt.pix_fmt)) {
		u32 num_cplanes =
		CIF_ISP10_PIX_FMT_YUV_GET_NUM_CPLANES(strm_fmt->frm_fmt.pix_fmt);
		if (num_cplanes == 0)
			strm_fmt->frm_fmt.stride =  f->fmt.pix.width *
			CIF_ISP10_PIX_FMT_GET_BPP(strm_fmt->frm_fmt.pix_fmt) / 8;
		else
			strm_fmt->frm_fmt.stride = f->fmt.pix.width;
	} else if (CIF_ISP10_PIX_FMT_IS_RGB(strm_fmt->frm_fmt.pix_fmt))
		strm_fmt->frm_fmt.stride =  f->fmt.pix.width *
		CIF_ISP10_PIX_FMT_GET_BPP(strm_fmt->frm_fmt.pix_fmt) / 8;

	f->fmt.pix.bytesperline = strm_fmt->frm_fmt.stride;
...
{
	struct cif_isp10_device *dev = to_cif_isp10_device(queue);
	enum cif_isp10_stream_id strm = to_cif_isp10_stream_id(queue->type);
	...
	struct v4l2_format *f)
{
	...
	int stream_id = to_stream_id(file);
	struct cif_isp10_mi_path_config *mi_config = NULL;
	int ret;

	cif_isp10_pltfrm_pr_dbg(NULL, "geting format\n");

	ret = cif_isp10_g_fmt(dev,
		stream_id, mi_config);
	if (IS_ERR_VALUE(ret))
		goto err;

	if (mi_config) {
		pix->pixelformat = cif_isp10_pix_fmt2v4l2_pix_fmt(mi_config->output.pix_fmt);
		pix->width = mi_config->output.width;
		pix->height = mi_config->output.height;
		pix->bytesperline = mi_config->llength;
...
static int cif_isp10_v4l2_try_fmt(
	struct file *file,
	void *fh,
	struct v4l2_format *f)
{
	...
	/* convert v4l2/cif format and check parameter */
	ret = cif_isp10_v4l2_fmt_strm_fmt(dev,
		&strm_fmt, f);
	...
	struct v4l2_frmsizeenum *fsize)
{
	...
	if (IS_ERR_OR_NULL(dev->img_src)) {
		cif_isp10_pltfrm_pr_err(NULL,
			"input has not yet been selected, cannot enumerate framesizes\n");
		ret = -ENODEV;
		goto err;
	}

	/* Look for the given pixel format */
	ifmt = get_cif_isp10_output_format_index(fsize->pixel_format);

	if (ifmt == get_cif_isp10_output_format_size())
		return -EINVAL;

	if (IS_ERR_VALUE(cif_isp10_img_src_enum_strm_fmts(dev->img_src,
		fsize->index, &strm_fmt_desc)))
		return -EINVAL;
...
static int mainpath_try_fmt_cap(struct v4l2_format *f)
{
	...
	for (ifmt = 0; ifmt < get_cif_isp10_output_format_size(); ifmt++) {
		if (pix->pixelformat == get_cif_isp10_output_format(ifmt)->fourcc)
			break;
	}
			     ...
			     struct v4l2_fmtdesc *f)
{
	...
	f->pixelformat = get_cif_isp10_output_format_desc(f->index)->pixelformat;
	...
	struct v4l2_ext_controls *vc_ext)
{
	...
	ctrl = kmalloc(sizeof(struct cif_isp10_img_src_ext_ctrl), GFP_KERNEL);
	if (!ctrl)
		return -ENOMEM;
	...
	struct v4l2_cropcap *a)
{
	...
	if ((dev->config.input_sel == CIF_ISP10_INP_DMA) ||
		...
		cif_isp10_pltfrm_pr_err(dev->dev,
			"cif_isp10_v4l2_cropcap: invalid input\n");
	}
